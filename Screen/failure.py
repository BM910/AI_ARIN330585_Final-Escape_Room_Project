import pygame
from data.icons.navigation import ICON_MENU, ICON_RESET
...
BTN_W, BTN_H    = 90, 48
BTN_GAP         = 16
BTN_TOTAL_W_2   = BTN_W * 2 + BTN_GAP
BTN_X_START     = BOX_X + (BOX_W - BTN_TOTAL_W_2) // 2
...
FAIL_ICON_CIRCLE_BG = (255, 220, 220)
FAIL_ICON_FG        = (180, 40,  40)
FAIL_TITLE_TEXT     = "Level failed!"
FAIL_TITLE_COLOR    = (160, 30,  30)
FAIL_DESC_TEXT      = "Don't give up — try again!"
...
class FailureScreen:
    ...
    def _draw_button(self, screen, rect, icon_matrix, accent=False):
    ...
    def draw(self, screen):
        ...
        pygame.draw.circle(screen, FAIL_ICON_CIRCLE_BG, (cx, icon_cy), ICON_CIRCLE_R)
        fail_surf = pygame.font.SysFont(FONT_EMOJI, ICON_CIRCLE_R).render("💀", True, FAIL_ICON_FG)
        screen.blit(fail_surf, fail_surf.get_rect(center=(cx, icon_cy)))

        title_surf = pygame.font.SysFont(FONT_NAME, TITLE_SIZE).render(
            FAIL_TITLE_TEXT, True, FAIL_TITLE_COLOR)
        screen.blit(title_surf, title_surf.get_rect(centerx=cx, top=icon_cy + TITLE_Y_OFFSET))

        desc_surf = pygame.font.SysFont(FONT_NAME, DESC_SIZE).render(
            FAIL_DESC_TEXT, True, DESC_COLOR)
        ...
        self._draw_button(screen, self.reset_button, ICON_RESET, accent=True)
